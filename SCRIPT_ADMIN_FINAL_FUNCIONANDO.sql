-- SCRIPT ADMIN FINAL QUE FUNCIONA
...
-- 7. Inserir na tabela profiles (sem status_disponibilidade por enquanto)
...
-- 11. Verificar se foi inserido na tabela profiles
SELECT 'PROFILE_CRIADO' as status, p.*, au.email 
FROM profiles p
JOIN auth.users au ON p.id = au.id
WHERE au.email = 'entregasobral@gmail.com';

-- 12. Verificar valores aceitos no status_disponibilidade (para referência futura)
SELECT 
    'VALORES_ACEITOS_STATUS' as info,
    enumlabel as valores_aceitos
FROM pg_enum 
WHERE enumtypid = (
    SELECT oid FROM pg_type WHERE typname = (
        SELECT udt_name FROM information_schema.columns 
        WHERE table_name = 'profiles' AND column_name = 'status_disponibilidade'
    )
);