-- Script para corrigir erro no sistema de chat
-- Erro: 42P01: missing FROM-clause entry for table "old"

-- Primeiro, vamos verificar se as tabelas existem
DO $$
BEGIN
    -- Verificar se a tabela conversas existe
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'conversas') THEN
        RAISE NOTICE 'Tabela conversas não existe, criando...';
    ...
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'mensagens') THEN
        RAISE NOTICE 'Tabela mensagens não existe, criando...';
...
DROP TRIGGER IF EXISTS create_order_conversation_trigger ON pedidos;
DROP FUNCTION IF EXISTS create_order_conversation();

-- Recriar função corrigida para criar conversa automaticamente para pedidos
...
BEGIN
  ...
  IF NEW.status = 'aceito' AND (OLD IS NULL OR OLD.status IS NULL OR OLD.status != 'aceito') THEN
...
DROP POLICY IF EXISTS "Users can view their conversations" ON conversas;
DROP POLICY IF EXISTS "Users can create conversations" ON conversas;
DROP POLICY IF EXISTS "Users can update their conversations" ON conversas;
DROP POLICY IF EXISTS "Admins can manage all conversations" ON conversas;

DROP POLICY IF EXISTS "Users can view messages from their conversations" ON mensagens;
DROP POLICY IF EXISTS "Users can send messages to their conversations" ON mensagens;
DROP POLICY IF EXISTS "Users can mark messages as read" ON mensagens;
DROP POLICY IF EXISTS "Admins can manage all messages" ON mensagens;

-- Recriar políticas de segurança para conversas
...
-- Recriar políticas de segurança para mensagens
...
DROP TRIGGER IF EXISTS update_conversation_last_message_trigger ON mensagens;
...
DO $$
BEGIN
    RAISE NOTICE 'Sistema de chat corrigido com sucesso!';
    RAISE NOTICE 'Tabelas criadas: conversas, mensagens';
    RAISE NOTICE 'Triggers criados: create_order_conversation_trigger, update_conversas_updated_at, update_mensagens_updated_at, update_conversation_last_message_trigger';
    RAISE NOTICE 'Políticas RLS criadas para ambas as tabelas';
END $$;