-- SCRIPT FINAL PARA DIAGNOSTICAR LOGIN DA MATUTARIA
-- Execute este script no Supabase SQL Editor

-- PRIMEIRO: Verificar estrutura das tabelas
SELECT 
    '0. ESTRUTURA PROFILES:' as etapa,
    column_name,
    data_type,
    is_nullable
...
-- 4. Verificar detalhes do perfil (apenas colunas que sabemos que existem)
SELECT 
...
-- 5. Verificar empresa (se existir relação)
SELECT 
...
-- 6. Verificar detalhes da empresa (se existir)
SELECT 
...
SELECT 
    ...
    CASE 
        ...
        WHEN p.role != 'empresa' THEN '❌ PROBLEMA: Role incorreto (' || COALESCE(p.role, 'NULL') || ')'
